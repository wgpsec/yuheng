...
export class BackupRepository extends RepositoryBase {
  ...
  private exportSnapshot(): FullBackupSnapshot {
    ...
    return {
      ...
      appSettings: rows("SELECT key, value, updated_at AS updatedAt FROM app_settings WHERE key IN ('browser_use', 'computer_use', 'desktop_presence', 'desktop_pet')").map((r) => ({ key: String(r.key), value: String(r.value), updatedAt: String(r.updatedAt) })),
  ...
  import(snapshot: FullBackupSnapshot): BackupImportReport {
    ...
    this.transaction(() => {
      ...
      for (const item of snapshot.conversations) {
        ...
        insertConversation.run(id, projectMap.get(item.projectId) ?? 'personal', item.title, item.description, item.archived ? 1 : 0, item.pinned ? 1 : 0, item.reasoningLevel, providerId, profileId, item.createdAt || now, item.updatedAt || now);
      }
