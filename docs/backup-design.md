...
6. Provider 仅按原 ID 匹配现有配置；不存在时绑定当前默认 Provider，并在导入报告中标记“需要重新配置”。不自动恢复 API Key。
...
- 应用退出时不等待自动备份，避免拖慢关闭；
- 自动备份使用同一 `.yuheng` 格式，不包含 SecretStore。
...
- API Key、MCP 凭据和 token 不进入 JSON、ZIP、日志、错误提示或导入报告；
