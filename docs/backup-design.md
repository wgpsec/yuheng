...
导出不读取或序列化 SecretStore 明文；Provider 只写非敏感配置。Pi session 在写入前按允许的文件清单复制，禁止把环境变量、Keychain 或临时 token 文件打包。
...
- 应用退出时不等待自动备份，避免拖慢关闭；
- 自动备份使用同一 `.yuheng` 格式，并按用户已授权的完整备份策略包含 Provider API Key；SecretStore 本身、Keychain 内容和原始密钥文件不复制。
...
- MCP 凭据和 token 不进入 JSON、ZIP、日志、错误提示或导入报告；完整备份中的 Provider API Key 是唯一例外，且只在用户明确导出时写入未加密 ZIP，导入后不进入数据库或日志；
