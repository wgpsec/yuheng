import { lazy, Suspense, useEffect, useMemo, useRef, useState, type CSSProperties, type DragEvent as ReactDragEvent, type FormEvent, type KeyboardEvent as ReactKeyboardEvent, type PointerEvent as ReactPointerEvent } from 'react';
...
function PetManagementSettings({ config, pets, assets, loading, onRefresh, onSelect, onToggle, onScaleChange, onLockedChange, onOpacityChange, onAlwaysOnTopChange, onFeedbackChange, onOpenFolder, onOpenExternal, onImport, onDelete, onReveal, error }: { config: DesktopPetConfig; pets: CodexPetCatalogEntry[]; assets: Record<string, { dataUrl: string; manifest: CodexPetManifest }>; loading: boolean; onRefresh: () => void; onSelect: (petId?: string) => void; onToggle: () => void; onScaleChange: (scale: number) => void; onLockedChange: (locked: boolean) => void; onOpacityChange: (opacity: number) => void; onAlwaysOnTopChange: (alwaysOnTop: boolean) => void; onFeedbackChange: (patch: Partial<DesktopPetConfig>) => void; onOpenFolder: () => void; onOpenExternal: (url: string) => void; onImport: () => void; onDelete: (petId: string) => void; onReveal: (petId: string) => void; error: string | null }) {
    ...
    <div className="pet-catalog" aria-label="宠物列表">
      ...
      {!loading && entries.map((pet) => {
        ...
        return <div className={`pet-catalog-row ${selected ? 'is-selected' : ''} ${!canUse ? 'is-invalid' : ''}`} key={pet.id || 'builtin'}>
          ...
          <div className="pet-catalog-copy"><strong>{pet.displayName}</strong><small>{pet.description ?? (pet.source === 'codex' ? '来自 Codex Pet 的本地皮肤。' : '玉衡内置宠物。')}</small>{!pet.builtin && <span className={`pet-compatibility-status is-${structuralIssue ? 'invalid' : fallbackOnly ? 'compatible' : pet.report.status}`}>{structuralIssue ? <AlertTriangle size={13} aria-hidden="true" /> : <Check size={13} aria-hidden="true" />}{statusLabel}{statusIssue ? ` · ${statusIssue.message}` : ''}</span>}</div>
      ...
      <div className="pet-catalog-resources" aria-label="更多宠物皮肤"><span>更多皮肤</span><div><button type="button" className="text-button" onClick={() => onOpenExternal('https://codex-pets.net/')}><ExternalLink size={14} aria-hidden="true" />Codex Pets</button><button type="button" className="text-button" onClick={() => onOpenExternal('https://codexpet.top/')}><ExternalLink size={14} aria-hidden="true" />Awesome Codex Pet</button></div></div>
    </div>
...
function SettingsWorkspace({ appInfo, current, providers, defaultProviderId, onDefaultProviderChange, browserUseConfig, computerUseConfig, browserEnvironment: initialBrowserEnvironment, computerEnvironment: initialComputerEnvironment, onBrowserEnvironmentChange, onComputerEnvironmentChange, desktopPetConfig: configuredDesktopPet, initialSection = 'provider', theme, onThemeChange, onClose, onSaved, onProvidersChange, onBrowserUseChange, onComputerUseChange, onDesktopPetChange }: { appInfo: AppInfo | null; current: ProviderConfig | null; providers: ProviderConfig[]; defaultProviderId: string | null; onDefaultProviderChange: (providerId: string) => void; browserUseConfig: BrowserUseConfig; computerUseConfig: ComputerUseConfig; browserEnvironment?: BrowserUseEnvironment | null; computerEnvironment?: ComputerUseEnvironment | null; onBrowserEnvironmentChange?: (environment: BrowserUseEnvironment) => void; onComputerEnvironmentChange?: (environment: ComputerUseEnvironment) => void; desktopPetConfig?: DesktopPetConfig; initialSection?: SettingsSection; theme: ThemeName; onThemeChange: (theme: ThemeName) => void; onClose: () => void; onSaved: (provider: ProviderConfig) => void; onProvidersChange: (providers: ProviderConfig[]) => void; onBrowserUseChange: (config: BrowserUseConfig) => void; onComputerUseChange: (config: ComputerUseConfig) => void; onDesktopPetChange?: (config: DesktopPetConfig) => void }) {
  ...
  const selectDefaultProvider = async (providerId: string) => {
    const activeBridge = getBridge();
    if (!activeBridge || !providerId || providerId === defaultProviderId) return;
    try {
      const saved = await activeBridge.provider.setDefault(providerId);
      onDefaultProviderChange(saved);
      setError(null);
    } catch (reason) { setError(reason instanceof Error ? reason.message : '更新默认 Provider 失败。'); }
  };
  const toggleBrowserUse = async () => {
    ...
    try {
      ...
      if (saved.enabled) onComputerUseChange(await activeBridge.computerUse.save({ enabled: false }));
  ...
  const toggleComputerUse = async () => {
    ...
    try {
      ...
      if (saved.enabled) onBrowserUseChange(await activeBridge.browserUse.save({ enabled: false }));
  ...
  return <section className="settings-layout" aria-label="玉衡设置">
    ...
    <section className="settings-page" aria-labelledby="settings-page-title">
      ...
      {activeSection === 'provider' && <div className="settings-section settings-section-first provider-settings-section">
        <div className="settings-section-heading provider-settings-heading"><div><h3>Provider 列表</h3><p>API Key 仅保存在这台 Mac 的钥匙串中。</p></div><button type="button" className="secondary-action" onClick={addProvider}>新增 Provider</button></div>
        <div className="provider-list-block">
          <div className="provider-list-label"><strong>已配置的 Provider</strong><span>{providers.length} 个</span></div>
          <div className="provider-profile-list">{providers.map((provider) => <div className={`provider-profile-row ${provider.id === editingProviderId ? 'is-selected' : ''}`} key={provider.id}><button type="button" onClick={() => selectProvider(provider)}><strong>{provider.displayName}</strong><small>{provider.protocol} · {provider.model}{provider.id === current?.id ? ' · 当前会话' : ''}{provider.id === defaultProviderId ? ' · 新会话默认' : ''}</small></button><button type="button" className="icon-button" onClick={() => void removeProvider(provider)} aria-label={`删除 ${provider.displayName}`}>×</button></div>)}</div>
        </div>
        <div className="provider-default-block">
          <div><strong>新会话默认 Provider</strong><small>只影响之后新建的会话；已有会话仍使用各自绑定的 Provider。</small></div>
          <select value={defaultProviderId ?? ''} onChange={(event) => void selectDefaultProvider(event.target.value)} disabled={providers.length === 0} aria-label="新会话默认 Provider"><option value="" disabled>请选择 Provider</option>{providers.map((provider) => <option value={provider.id} key={provider.id}>{provider.displayName}{provider.hasApiKey ? '' : ' · 未配置 API Key'}</option>)}</select>
        </div>
        <div className="provider-editor-block">
          <div className="provider-editor-heading"><strong>{editingProviderId ? '编辑 Provider' : '新增 Provider'}</strong><span>{editingProviderId ? '修改后保存即可生效' : '填写信息后保存'}</span></div>
        <form onSubmit={save}>
...
export function ProductionRenderer() {
  ...
  const { appInfo, provider, setProvider, providers, setProviders, defaultProviderId, setDefaultProviderId, browserUse: browserUseConfig, setBrowserUse: setBrowserUseConfig, computerUse: computerUseConfig, setComputerUse: setComputerUseConfig, theme, setTheme } = settings;
  ...
  const createConversation = async (projectId = activeConversationProject, profileId = DEFAULT_AGENT_PROFILE_ID) => {
    closeSettings();
    setComposerPrefill(null);
    if (!activeBridge) return selectConversation('inbox');
    try { const created = await activeBridge.conversations.create(undefined, projectId, undefined, profileId); setConversationItems((items) => sortConversations([created, ...items])); setActiveConversation(created.id); setActiveConversationProject(created.projectId); setActiveProfileId(created.profileId); setProvider(providers.find((item) => item.id === created.providerId) ?? null); openWorkspaceTab('conversation', created.id); }
  ...
  const runTemporaryAi = async (title: string, prompt: string, signal?: AbortSignal): Promise<string> => {
    ...
    const temporary = await activeBridge.conversations.create(title, activeConversationProject, activeProviderId || undefined, activeProfileId);
  ...
  const createConversationProject = async (name: string, workspacePath?: string | null): Promise<void> => {
    if (!activeBridge) return;
    const created = await activeBridge.conversations.projects.create(name, workspacePath);
  ...
  const setConversationProjectWorkspace = async (id: string, workspacePath: string | null): Promise<void> => {
    if (!activeBridge) return;
    const updated = await activeBridge.conversations.projects.setWorkspace(id, workspacePath);
  ...
  const handleAttachmentDragOver = (event: ReactDragEvent<HTMLElement>) => {
    if (settingsMounted || activeView !== 'conversation' || !event.dataTransfer.types.includes('Files')) return;
    event.preventDefault();
    event.dataTransfer.dropEffect = 'copy';
    setAttachmentDropActive(true);
  };
  const handleAttachmentDragLeave = (event: ReactDragEvent<HTMLElement>) => {
    if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setAttachmentDropActive(false);
  };
  const handleAttachmentDrop = (event: ReactDragEvent<HTMLElement>) => {
    if (settingsMounted || activeView !== 'conversation' || !event.dataTransfer.types.includes('Files')) return;
    event.preventDefault();
    setAttachmentDropActive(false);
    if (!activeBridge) return;
    const paths = Array.from(event.dataTransfer.files).map((file) => {
      try { return activeBridge.attachments.pathForFile(file); } catch { return ''; }
    }).filter((filePath): filePath is string => Boolean(filePath));
    if (paths.length === 0) { setError('无法读取拖放文件，请改用附件按钮选择。'); return; }
    void activeBridge.attachments.addPaths(paths).then((selected) => {
      setAttachments((current) => [...current, ...selected.filter((item) => !current.some((existing) => existing.id === item.id))]);
      setError(null);
    }).catch((reason) => { setError(reason instanceof Error ? reason.message : '添加拖放附件失败。'); });
  };
  ...
  const saveConversationCapability = async (capability: keyof ConversationCapabilities, value: ConversationCapabilityOverride) => {
    ...
    const conversationId = activeConversation;
    try {
      const next = { ...conversationCapabilities, [capability]: value };
      if (value === 'enabled') next[capability === 'browserUse' ? 'computerUse' : 'browserUse'] = 'disabled';
      const saved = await activeBridge.conversations.capabilities.save(conversationId, next);
  ...
  return <main className={`app-shell ${sidebarCollapsed ? 'sidebar-is-collapsed' : ''} ${sidebarResizing ? 'sidebar-is-resizing' : ''} ${activeView === 'tasks' || activeView === 'notes' ? 'tasks-is-active' : ''} ${activeView === 'notes' ? 'notes-is-active' : ''} ${settingsMounted ? 'settings-is-active' : ''} ${settingsClosing ? 'settings-is-closing' : ''} ${profileMenuClosing || providerMenuClosing ? 'header-menu-closing' : ''} ${attachmentDropActive ? 'attachment-drop-active' : ''}`} style={{ '--sidebar-width': `${sidebarWidth}px` } as CSSProperties} onDragOver={handleAttachmentDragOver} onDragLeave={handleAttachmentDragLeave} onDrop={handleAttachmentDrop}>
    ...
    {attachmentDropActive && activeView === 'conversation' && !settingsMounted && <div className="attachment-drop-overlay" role="status">松开以上传附件</div>}
