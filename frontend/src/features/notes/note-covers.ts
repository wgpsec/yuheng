...
export const BUILT_IN_NOTE_COVERS: readonly BuiltInNoteCover[] = [
  { id: 'aurora', label: '极光', background: 'linear-gradient(120deg, #173b4f 0%, #397f78 48%, #b7c98d 100%)', image: new URL('../../assets/note-covers/aurora.webp', import.meta.url).href },
  ...
  { id: 'lavender', label: '暮紫', background: 'linear-gradient(120deg, #34294f 0%, #75649c 50%, #d7bfdc 100%)', image: new URL('../../assets/note-covers/lavender.webp', import.meta.url).href },
  { id: 'sand', label: '砂岩', background: 'linear-gradient(120deg, #6a4937 0%, #b7835d 50%, #e4c79e 100%)', image: new URL('../../assets/note-covers/sandstone.webp', import.meta.url).href },
...
export function noteCoverBackgroundStyle(background?: string, image?: string): NoteCoverBackgroundStyle {
  return {
    ...((image || background) ? { backgroundImage: [image ? `url(${image})` : '', background].filter(Boolean).join(', ') } : {}),
    backgroundPosition: 'center',
    backgroundSize: 'cover',
    backgroundRepeat: 'no-repeat',
  };
