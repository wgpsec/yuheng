...
import { builtInNoteCover, BUILT_IN_NOTE_COVERS, noteCoverBackgroundStyle } from './note-covers';
...
export function NotesWorkspace({ bridge, initialNoteId, refreshKey = 0, showNavigation = true, onNotesChanged, onActiveNoteTitleChange, onActiveNoteChange, onAskAi, onRunAi, onCreateTask, onOpenTask, onImportAsset, onPickAssets, onOpenAsset }: NotesWorkspaceProps) {
  ...
  const [coverPreviewRatio, setCoverPreviewRatio] = useState(16 / 5);
  ...
  useEffect(() => { setAiPreview(null); setUndoContent(null); setPageMenuOpen(false); setAiNotice(null); setEmojiPickerOpen(false); setCoverPickerOpen(false); setHistoryOpen(false); setNoteVersions([]); setPeekNoteId(null); setOutlineOpen(false); setActiveOutlineIndex(0); }, [activeId]);
  useEffect(() => {
